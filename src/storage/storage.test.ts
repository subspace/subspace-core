...
const storageTest = (adapterName: string, namespace?: string) => {
  describe(`Running storage tests for ${adapterName}`, () => {
    let storage: Storage;

    beforeAll(async () => {
      storage = await Storage.create(adapterName, namespace);
    });

    test(`${adapterName}-put`, async () => {
      await storage.put(k0, v0);
    });

    test(`${adapterName}-get`, async () => {
    ...
    test(`${adapterName}-put-get-many`, async () => {
    ...
    test(`${adapterName}-get-keys`, async () => {
    ...
    test(`${adapterName}-get-length`, async () => {
      const length = await storage.getLength();
      expect(length).toBe(3);
    });

    test(`${adapterName}-clear`, async () => {
      await storage.clear();
      const keys = await storage.getKeys();
